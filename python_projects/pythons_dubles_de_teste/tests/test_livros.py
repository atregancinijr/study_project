...
class StubHTTPResponse:
    ...
    # como a funcao é chamada dentro de um contexto eu tenho que ter __exit__ e __enter__
    def __enter__(self):
        return self
...
def stub_de_urlopen_que_levanta_excecao_http_error(url, timeout):
    fp = unittest.mock.mock_open
    fp.close = Dummy
    raise HTTPError(Dummy(), Dummy(), "mensagem de erro", Dummy(), fp)


@unittest.mock.patch("colecao.livros.urlopen", return_value=StubHTTPResponse())
# @unittest.skip("Vale quando consultar_livros estiver completo")
def test_consultar_livros_retorna_resultado_formato_string(duble_urlopen):
...
def test_consultar_livros_chama_preparar_dados_para_requisicao_uma_vez_e_com_os_mesmos_parametros_de_consultar_livros(
        stub_urlopen):
    with unittest.mock.patch("colecao.livros.preparar_dados_para_requisicao") as spy_preparar_dados:
        consultar_livros("Agatha Christie")
        spy_preparar_dados.assert_called_once_with("Agatha Christie")
        #  Spy, verifica se o que ele está recebendo de parâmetro é o que é esperado


@unittest.mock.patch("colecao.livros.urlopen", return_value=StubHTTPResponse())
def test_consultar_livros_chama_funcao_obter_url_usando_como_parametro_o_retorno_de_preparar_dados_para_requisicao(
        stub_urlopen):
    with unittest.mock.patch("colecao.livros.preparar_dados_para_requisicao") as stub_preparar:
        dados = {"autor": "Agatha Christie"}
        stub_preparar.return_value = dados
        # Stub porque ele está retornando um valor pré-definido.
        with unittest.mock.patch("colecao.livros.obter_url") as spy_obter_url:
            consultar_livros("Agatha Christie")
...
@unittest.mock.patch("colecao.livros.urlopen", return_value=StubHTTPResponse())
def test_consultar_livrods_chama_executar_requisicao_usando_retorno_obter_url(stub_urlopen):
    with unittest.mock.patch("colecao.livros.obter_url") as stub_obter_url:
        stub_obter_url.return_value = "https://buscador"
        with unittest.mock.patch("colecao.livros.executar_requisicao") as spy_executar_requisicao:
            consultar_livros("Agatha Christie")
            spy_executar_requisicao.assert_called_once_with("https://buscador")


def test_executar_requisicao_retorna_tipo_string():
    with unittest.mock.patch("colecao.livros.urlopen", stub_de_urlopen):
        print(stub_de_urlopen)
        resultado = executar_requisicao("https://buscarlivros?author=Jk+Rowlings")
    assert type(resultado) == str


def test_executar_requisicao_retorna_resultado_tipo_string():
    # mesmo exemplo anterior, só que agora é um objeto da biblioteca mock, no anterior era uma função
    with unittest.mock.patch("colecao.livros.urlopen") as duble_de_url_open:
        print(duble_de_url_open)
        duble_de_url_open.return_value = StubHTTPResponse()
        resultado = executar_requisicao("https://buscarlivros?author=Jk+Rowlings")
        assert type(resultado) == str


def test_executar_requisicao_retorna_resultado_tipo_string_variacao2():
    with unittest.mock.patch("colecao.livros.urlopen", return_value=StubHTTPResponse()):
        resultado = executar_requisicao("https://buscarlivros?author=Jk+Rowlings")
        assert type(resultado) == str


@unittest.mock.patch("colecao.livros.urlopen", return_value=StubHTTPResponse())
def test_executar_requisicao_retorna_resultado_tipo_string_variacao3(duble_de_urlopen):
    resultado = executar_requisicao("https://buscarlivros?author=Jk+Rowlings")
    assert type(resultado) == str


@unittest.mock.patch("colecao.livros.urlopen")
def test_executar_requisicao_retorna_resultado_tipo_string_variacao4(duble_de_urlopen):
...
def test_executar_requisicao_levanta_excecao_do_tipo_http_error():
    with unittest.mock.patch("colecao.livros.urlopen", stub_de_urlopen_que_levanta_excecao_http_error):
...
@unittest.mock.patch("colecao.livros.urlopen")
def test_executar_requisicao_levanta_excecao_do_tipo_http_error_variacao2(duble_de_urlopen):
    fp = unittest.mock.mock_open
    fp.close = Dummy
...
@unittest.mock.patch("colecao.livros.urlopen")
def test_executar_requisicao_levanta_excecao_do_tipo_http_error_variacao3(duble_de_urlopen):
    fp = unittest.mock.mock_open
    myMock = unittest.mock.Mock()
    fp.close = myMock
    duble_de_urlopen.side_effect = HTTPError(myMock, myMock, "mensagem de erro", myMock, fp)
    with pytest.raises(HTTPError) as excecao:
        executar_requisicao("https://")
        assert "mensagem de erro" in str(excecao.value)
...
def test_executar_requisicao_loga_mensagem_de_erro_de_http_error(caplog):
    with unittest.mock.patch("colecao.livros.urlopen", stub_de_urlopen_que_levanta_excecao_http_error):
        resultado = executar_requisicao2("http://")
...
@unittest.mock.patch("colecao.livros.urlopen")
def test_executar_requisicao_loga_mensagem_de_erro_de_http_error_variacao2(stub_de_urlopen, caplog):
    fp = unittest.mock.mock_open
    myMock = unittest.mock.Mock()
    fp.close = myMock
    stub_de_urlopen.side_effect = HTTPError(myMock, myMock, "mensagem de erro", myMock, fp)
    executar_requisicao2("http://")
    mensagem_de_erro = "mensagem de erro"
    assert len(caplog.records) == 1
    for registro in caplog.records:
        assert mensagem_de_erro in registro.message
...
def test_escrever_em_arquivo_registra_excecao_que_nao_foi_possivel_criar_diretorio():
    ...
    with unittest.mock.patch("colecao.livros.os.makedirs", duble_makedirs):
        with unittest.mock.patch("colecao.livros.logging", duble_logging):
            escrever_em_arquivo(arquivo, conteudo)
            assert "Não foi possível criar diretório /tmp" in duble_logging.mensagens


@unittest.mock.patch("colecao.livros.os.makedirs")
@unittest.mock.patch("colecao.livros.logging.exception")
@unittest.mock.patch("colecao.livros.open", side_effect=OSError()) # side_effect ->levanta exceção
...
@unittest.mock.patch("colecao.livros.open")
...
@unittest.mock.patch("colecao.livros.open")
def test_escrever_em_arquivo_chama_write_usando_a_biblioteca_spy_do_mock(stub_de_open):
    arq = "/bla/arquivo.json"
    conteudo = "conteudo do arquivo"
    spy_de_fp = unittest.mock.MagicMock()
...
def test_registrar_livros_insere_12_registros_na_base_de_dados(stub_ler_arquivo, resultado_em_tres_paginas):
    ...
    qtd = registrar_livros(arquivos, fake_db.inserir_registros)
    assert qtd == 8
    assert fake_db.registros[0] == {"author": "Luciano Ramalho",
                 "title": "Python Fluente"}