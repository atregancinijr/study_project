...
fig, axes =  plt.subplots(1, figsize = (9.5, 7.5))

plt.xlabel(r'$\textrm{BER}_{\textrm{in}}$', fontsize = 16)
plt.ylabel(r'$\textrm{BER}_{\textrm{out}}$', fontsize = 16)

#plt.title(f"BIBD - {code[0]}", y = 1.02)


BER_theory = 0.5 * erfc(np.sqrt(10 ** (np.array(SNR_list) / 10)))
plt.ylim(10**-6, 10**-1)
plt.xlim(min(BER_theory), max(BER_theory))
axes.semilogy(BER_theory, BER_theory, '--r', label='Sem codificação')
for i, x in enumerate(H_list_results_iterations):
    for j in range(0, len(code)):
        plt.loglog(BER_theory, x, color=color_list[j], linestyle='-', marker=marker_list[i],
                     label=f'Iteração = {iteration_list[i]}')

formatter = tck.LogFormatterSciNotation(base=10, labelOnlyBase=False, minor_thresholds=(2, 0.4))
