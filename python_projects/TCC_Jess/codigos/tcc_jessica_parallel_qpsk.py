...
def add_noise(input_signal, SNR_dB):
    x = np.copy(input_signal)   #mapeamento de binário para BPSK
    ...
    if isinstance(x[0], complex):
        n = np.sqrt(N0/2)*(np.random.randn(*x.shape) + 1j*np.sqrt(N0/2)*np.random.randn(*x.shape)) #não tenho certeza
    else:
...
def QPSK_modulation(x):
    mod_x = np.array(x)
    temp = mod_x.shape
    bit_num = temp[0]
    #print(bit_num)
    TX_BPSK = np.ones((1, bit_num))

    TX_BPSK[mod_x[0] == 0] = -1
    TX_BPSK = TX_BPSK * 1 / np.sqrt(2)
    TX_QPSK = TX_BPSK[0, 0:bit_num:2] + 1j * TX_BPSK[0, 1:bit_num:2]

    return TX_QPSK

def QPSK_demodulation(mod_x):
    y = np.copy(mod_x)
    x = md.QPSK_to_bit(np.array([y]))
    return x[0].astype(int)

def separate_InPhase_Quadrature(modulated_signal):
    in_phase = np.imag(modulated_signal)
    quadrature = np.real(modulated_signal)
    return [in_phase, quadrature]

def my_function(iteration, H_list, SNR_list, maxiter):
    ...
    for i, H in enumerate(H_list):
        ...
        for SNR_dB in SNR_list:
            ...
            for ii in range(0, maxiter):
                ...
                v = np.hstack([v, v])
                #print(f'Mensagem codificada: {v}, {np.shape(v)}')
                #modulando em QPSK:
                v2 = QPSK_modulation(v)
                #print(np.shape(v2))
                #print(f'Mensagem modulada em QPSK: {v2}')
                y = add_noise(v2, SNR_dB)
                #print(f'Mensagem + ruído: {y}')
                #Separa a parte in phase e quadrature
                [y_inphase, y_quadrature] = separate_InPhase_Quadrature(y)
                #print(f'in_phase: {y_inphase}')
                #print(f'quadrature: {y_quadrature}')
                x_inphase = decode_sum_product(y_inphase, H, iteration=iteration, SNR_dB=SNR_dB)
                x_quadrature = decode_sum_product(y_quadrature, H, iteration=iteration, SNR_dB=SNR_dB)
                x_inphase=BPSK_modulation(x_inphase)
                x_quadrature=BPSK_modulation(x_quadrature)
                x_qpsk = (1/np.sqrt(2))*(x_quadrature + 1j*x_inphase)
                #print(f'remount qpsk: {x_qpsk}')
                x=QPSK_demodulation(x_qpsk)
                #print(f'qpsk_demodulated: {x}')
                u_linha = np.copy(x)#get_message(G, x)
                ...
                print(f'Numero de erros do "sum product" para {SNR_dB} dB de SNR : {sum(abs(v - u_linha))}')
...
if __name__=='__main__':
    ...
    # bbdi = BIBD_class.BIBD(tc=3, tr=23)
    bbdi = BIBD_class.BIBD(tc=3, tr=23)
    mgallager = Gallager_class.Matriz_de_gallager(23 ** 2, 3, 23)
    ...
    maxiter = 10
    SNR_list = [5.5, 6.0, 6.5]
    color_list = ['b', 'k']
    marker_list = ['o', '^', 'x']
    iteration_list = [2]

    H_list_results_iterations = poolv.starmap(my_function,
                                              [(iteration, H_list, SNR_list, maxiter) for iteration in iteration_list])
    ...
    for i, x in enumerate(H_list_results_iterations):
        for j in range(0, len(H_list)):
            plt.semilogy(SNR_list, x[j][1][0], color=color_list[j], linestyle='--', marker=marker_list[i],
                         label=f'{x[j][0]} Sum Product - Iteration = {iteration_list[i]}')
            plt.legend()
    plt.show()
